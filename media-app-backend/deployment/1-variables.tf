...
# command in CLI to get VPC Availability Zones
...
variable "main_api_server_domain" {
  ...
  type        = string
  default     = "mediappserver.online" # Your backend domain you created a route53 zone for
}

variable "dev_api_server_domain" {
  description = "Dev API Server Domain"
  type        = string
  default     = "api.dev.mediappserver.online"
}

variable "ec2_iam_role_name" {
  description = "EC2 IAM Role Name"
  type        = string
  default     = "mediapp-server-ec2-role" # Add a unique name
}

variable "ec2_iam_role_policy_name" {
  description = "EC2 IAM Role Policy Name"
  type        = string
  default     = "mediapp-server-ec2-role-policy" # Add a unique name
}

variable "ec2_instance_profile_name" {
  description = "EC2 Instance Profile Name"
  type        = string
  default     = "mediapp-server-ec2-instance-profile" # Add a unique name
}
...
variable "code_deploy_role_name" {
  ...
  type        = string
  default     = "mediapp-server-codedeploy-role" # Add a unique name
}

variable "prefix" {
  description = "Prefix to be added to AWS resources tags"
  type        = string
  default     = "mediapp-server" # Add a unique identifier name
}

variable "project" {
  description = "Prefix to be added to AWS resources local tags"
  type        = string
  default     = "mediapp-server" # You can use the name unique identifier created above
}
